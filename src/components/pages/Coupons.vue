...
<script>
import $ from "jquery";
import { getCoupons, updateCoupon, addCoupon, delCoupon } from "@/api/coupons";
...
export default {
  ...
  methods: {
    ...
    getCoupon() {
      const vm = this;
      vm.isLoading = true;
      getCoupons(vm.tempProduct).then((response) => {
    ...
    updateCoupon() {
      const vm = this;
      if (vm.isNew) {
        addCoupon(vm.tempCoupon).then((response) => {
          console.log(response);
